...
function listingListener(dex) {
  ...
  switch (dex) {
    ...
    case 'pancake':
      ...
      break;
    case 'quickswap':
      provider = new ethers.providers.WebSocketProvider(process.env.POLYGON_NODE);
  ...
  contract.on('PairCreated', async (token0, token1, pair, listingNumber) => {
    const tokenContract0 = new ethers.Contract(token0, config.GENERIC_ERC20_ABI, provider);
    const tokenContract1 = new ethers.Contract(token1, config.GENERIC_ERC20_ABI, provider);
    tokenContract0.n
    const [name0, name1 ] = await Promise.all([tokenContract0.name(), tokenContract1.name()]);
    console.log(new Date().toISOString().split('.')[0], listingText, token0, token1, pair, '(' + name0 + ' - ' + name1 + ')');
    console.log('\n');
